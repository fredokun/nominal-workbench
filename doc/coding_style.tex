...
\item \textbf{File names}: The name of any files (e.g. .ml or .mli) must be in
  lower case.
\item \textbf{Module name}: Use the camel case.
\item \textbf{Sum types and exceptions}: They should be written as module's names.
\end{itemize}
...
\item \textbf{Module opening}: Try to minimize the dependencies, if you are using a module in one function, use the let open.
